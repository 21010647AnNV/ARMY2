...
public class BangXHManager {
    ...
    protected static final String[] bangXHString = new String[]{"DANH DỰ", "CAO THỦ", "ĐẠI GIA XU", "ĐẠI GIA LƯỢNG","CHUYỂN SINH","ĐIỂM"};
    protected static final String[] bangXHString1 = new String[]{"Danh dự", "xp", "xu", "lượng", "lần", "Điểm"};
    ...
    protected static void refreshXH(int type) {
        ...
        switch (type) {
            case 0:
                try {
                    ResultSet res = SQLManager.stat.executeQuery("SELECT `id`,`dvong` FROM `armymem` ORDER BY `dvong` DESC LIMIT 0, 100;");
                    int i = 1;
                    while (res.next()) {
                    ...
                    for (int j = 0; j < bxh.size(); j++) {
                        ...
                        SQLManager.stat.executeUpdate("UPDATE `armymem` SET `top`='" + bXHE.index + "' WHERE `id`=" + bXHE.iddb + " LIMIT 1;");
                        int[] xuUp = new int[]{5, 4, 3};
                        if (bXHE.index <= 3) {
                            if (ServerManager.getUser(bXHE.iddb) != null) {
                                ServerManager.getUser(bXHE.iddb).updateXu(xuUp[bXHE.index - 1]);
                            } else {
                                SQLManager.stat.executeUpdate("UPDATE `armymem` SET `xu`= `xu` + " + xuUp[bXHE.index - 1] + " WHERE (`id`=" + bXHE.iddb + " AND `xu` < 2000000000)LIMIT 1;");
                            }
                        }
                        if (j > 99) {
                            bxh.remove(j);
            ...
            Logger.getLogger(BangXHManager.class.getName()).log(Level.SEVERE, null, ex);
        }
                break;
            ...
            case 4:
                ...
                try {
                    int i = 1;
                    ResultSet red = SQLManager.stat.executeQuery("SELECT `id`,`CSinh` FROM `armymem` ORDER BY `CSinh` DESC LIMIT 0, 100;");
                    while (red.next()) {
                        int iddb = red.getInt("id");
                        int point = red.getInt("CSinh");
                        if (iddb < 3 || point < 1) {
                            continue;
                        }
                        BangXHEntry bXHE = new BangXHEntry();
            ...
            case 5:
                ...
                try {
                    int i = 1;
                    ResultSet red = SQLManager.stat.executeQuery("SELECT `id`,`sk` FROM `armymem` ORDER BY `sk` DESC LIMIT 0, 100;");
                    while (red.next()) {
                        int iddb = red.getInt("id");
                        int point = red.getInt("sk");
                        if (iddb < 3 || point < 1) {
                            continue;
                        }
                        BangXHEntry bXHE = new BangXHEntry();
    ...
    public static void refreshTopTeam() {
        try {
            ClanEntry clan = null;
            ResultSet red = SQLManager.stat.executeQuery("SELECT * FROM `clan` ORDER BY `xp` DESC, `cup` DESC LIMIT 100;");
            while (red.next()) {
                clan = new ClanEntry();
