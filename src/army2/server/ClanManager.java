...
public class ClanManager {
    ...
    protected static void getMemberClan(User us, Message ms) {
        try {
            ...
            int numPage;
            try (ResultSet res = SQLManager.stat.executeQuery(String.format("SELECT `Mem` FROM `clan` WHERE `id` = %d;", Ids))) {
                res.first();
                int mem = res.getInt("Mem");
                numPage = (mem % 10 == 0) ? mem / 10 : mem / 10 + 1;
            }
            if (page >= numPage) {
                page = 0;
            }
            ...
            int CupClan;
            try (ResultSet red = SQLManager.stat.executeQuery(String.format("SELECT `clanmem`.*, `armymem`.*, `user`.`user` FROM `clanmem` INNER JOIN `armymem` ON `clanmem`.`user` = `armymem`.`id` INNER JOIN `user` ON `clanmem`.`user` = `user`.`user_id` WHERE `clanmem`.`clan` = %d ORDER BY `clanmem`.`rights` DESC, `clanmem`.`xp` DESC LIMIT %d, 10;", Ids, (page * 10)))) {
                ...
                while (red.next()) {
                    ...
                    memberEntry.setTimeJoin(red.getString("clanmem.timeJoin"));
            ...
            for (int i = 0; i < length; i++) {
                ...
                ds.writeUTF(memClan.getN_contribute() > 0 ? (memClan.getN_contribute() + " lần: " + Until.getStrungNum(memClan.getXu()) + " xu và " + Until.getStrungNum(memClan.getLuong()) + " lượng") : "");
            }
            member.clear();
            SQLManager.stat.executeUpdate("UPDATE `clan` SET `cup` =  " + CupClan + "  WHERE `id` = " + Ids + ";");
            ds.flush();
            us.sendMessage(ms);
    ...
    protected static void clanItemMessage(User us, Message ms) {
        try {
            ...
            if (idClan > 0) {
                ResultSet red;
                red = SQLManager.stat.executeQuery("SELECT * FROM `clan` WHERE `id` = '" + idClan + "' LIMIT 1;");
                if (!red.first()) {
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF(GameString.clanNull());
    ...
    public static int getXuClan(User us) {
        ...
        if (us.getClan() > 0) {
            try {
                ResultSet red;
                red = SQLManager.stat.executeQuery("SELECT * FROM `clan` WHERE `id` = '" + us.getClan() + "' LIMIT 1;");
                red.first();
    ...
    public static void updateXP(User us, int xp) {
        ...
        if (us.getClan() > 0) {
            ...
            try {
                ResultSet red;
                red = SQLManager.stat.executeQuery("SELECT `xp` FROM `clan` WHERE `id` = '" + us.getClan() + "' LIMIT 1;");
                if (red.first()) {
                    int level = (int) (((int) Math.sqrt(1 + red.getInt("xp") / 6250) + 1) >> 1);
                    level = level > 127 ? 127 : level;
                    SQLManager.stat.executeUpdate("UPDATE `clan` SET `level` = " + level + ",`xp` = `xp` + " + xp + "  WHERE `id` = " + us.getClan() + ";");
                    SQLManager.stat.executeUpdate("UPDATE `clanmem` SET `xp` = `xp` + " + xp + " WHERE `user` = " + us.getIDDB() + ";");
                }
                red.close();
            } catch (SQLException e) {
            }
    ...
    public static void updateXu(User us, int xuUp) {
        ...
        try {
            SQLManager.stat.executeUpdate("UPDATE `clan` SET  `xu` = `xu` + " + xuUp + " WHERE `id` = " + us.getClan() + ";");
            if (xuUp > 0) {
                SQLManager.stat.executeUpdate("UPDATE `clanmem` SET `xu` = `xu` + " + xuUp + " WHERE `user` = " + us.getIDDB() + ";");
            }
        } catch (SQLException e) {
        }
    }
    ...
    public static void updateLuong(User us, int luongUp) {
        ...
        try {
            SQLManager.stat.executeUpdate("UPDATE `clan` SET  `luong` = `luong` + " + luongUp + " WHERE `id` = " + us.getClan() + ";");
            if (luongUp > 0) {
                SQLManager.stat.executeUpdate("UPDATE `clanmem` SET `luong` = `luong` + " + luongUp + " WHERE `user` = " + us.getIDDB() + ";");
            }
        } catch (SQLException e) {
        }
    }
    ...
    protected static void updateItemClan(User us, int Ids, int time) {
        if (us.getClan() > 0) {
            ...
            try {
                ResultSet red;
                red = SQLManager.stat.executeQuery("SELECT `Item` FROM `clan` WHERE `id` = " + us.getClan() + " LIMIT 1;");
                red.first();
                JSONArray itemClan = (JSONArray) JSONValue.parse(red.getString("item"));
                ...
                red.close();
                SQLManager.stat.executeUpdate("UPDATE `clan` SET `item`='" + itemClan.toJSONString() + "' WHERE `id`=" + us.getClan() + " LIMIT 1;");
            } catch (SQLException e) {
            }
        }
    }
        ...
        if (us.getClan() > 0) {
            try {
                ...
                if (type == 0) {
                    ...
                    updateXu(us, soluong);
                    us.updateXu(-soluong);
                    SQLManager.stat.executeUpdate("UPDATE `clanmem` SET `n_contribute` = `n_contribute` + 1, `contribute_time` = '" + Until.toDateString(new Date()) + "', `contribute_text` = '" + Until.getStrungNum(soluong) + " xu" + "' WHERE `user` = " + us.getIDDB() + ";");
                } else if (type == 1) {
                    ...
                    SQLManager.stat.executeUpdate("UPDATE `clanmem` SET `n_contribute` = `n_contribute` + 1, `contribute_time` = '" + Until.toDateString(new Date()) + "', `contribute_text` = '" + Until.getStrungNum(soluong) + " lượng" + "' WHERE `user` = " + us.getIDDB() + ";");
                }
                ms = new Message(45);
                DataOutputStream ds = ms.writer();
