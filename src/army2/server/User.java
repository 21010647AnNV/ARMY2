...
public class User {
   ...
   // private String report;
    ...
    /* public synchronized void updateXukhoa(int xuUp) throws IOException {
        if(xuUp==0){
            return;
        }
      /*  long sum = (long) xuUp + (long) this.xukhoa;
        if (sum > Integer.MAX_VALUE) {
            this.xukhoa = Integer.MAX_VALUE;
        } else if (sum < Integer.MIN_VALUE) {
            this.xukhoa = Integer.MIN_VALUE;
        } else {
            this.xukhoa += xuUp;
        }  
        if(xuUp>0)this.xukhoa+=xuUp;
        else if(xuUp<0){
            if(Math.abs(xuUp)<=this.xukhoa)
                this.xukhoa+=xuUp;
            else if(Math.abs(xuUp)<=this.xukhoa+this.xu){
                this.xu += (this.xukhoa+xuUp);
                this.xukhoa=0;
            } else 
                this.xu+=xuUp;
        ...
        ds.writeInt(this.xu+this.xukhoa);
    ...
    public synchronized void updateXP(int addXP, boolean canX2) {
        try {
            if (canX2 && addXP > 0) {
               ...
               if(hours >= 8 && hours <= 12) // X2 Exp từ 8h tối tới 12h tối
                {
                  addXP *= 2;
                }
            }
            ...
            if (this.nvCSinh[this.nv] == 0 && nextLevel > 2000) {
                nextLevel = 2000;
            } else if(nextLevel > 2000) {
                nextLevel = 2000;
            }
        ...
        if (updateLevel) {
            ds.writeByte(1);
            int lever = this.lever[this.nv];
            if (client.versionARM.startsWith("2.3.0")) {
                ds.writeByte(lever < 2000 ? lever : 2000);
            } else {
                ds.writeInt(lever);
            }
    ...
    protected static User login(ClientEntry cl, String user, String pass) {
        try {
            ...
            if (!BangXHManager.isComplete) {
                ...
                DataOutputStream ds = ms.writer();
                ds.writeUTF("Server đang bảo trì, Vui lòng quay lại sau 10p");
                ds.flush();
                cl.sendMessage(ms);
                return null;
            }
             if (!cl.versionARM.startsWith("2.2.3") && !cl.versionARM.startsWith("2.3.0")) {
                Message ms = new Message(86);
                DataOutputStream ds = ms.writer();
                ds.writeUTF("Vui lòng tải phiên bản mới!");
                ds.writeUTF(ServerManager.taiGameURL);
                ds.writeByte(1);
                ds.flush();
                cl.sendMessage(ms);
                return null;
            ...
            if (red.first()) {
               ...
               // byte active = red.getByte("active");
                red.close();
               /* if (active == 0) {
                    Message ms = new Message(4);
                    DataOutputStream ds = ms.writer();
                    ds.writeUTF("Vui lòng kiểm tra email để kích hoạt tài khoản!");
                    ds.flush();
                    cl.sendMessage(ms);
                    Thread.sleep(10000);
                    cl.close();
                ...
                if (lock) {
                    Message ms = new Message(4);
                    DataOutputStream ds = ms.writer();
                    ds.writeUTF(String.format(GameString.userLoginLock(), us.username));
                    ds.flush();
                    cl.sendMessage(ms);
                    Thread.sleep(10000);
                    cl.close();
                ...
                if (!red.first()) {
                    SQLManager.stat.executeUpdate("INSERT INTO armymem(`id`, `xu`, `luong`, `ruongItem`, `ruongTrangBi`) VALUES (" + us.iddb + ", 1000000,1000, '[]', '[]');");
                    red.close();
                    red = SQLManager.stat.executeQuery("SELECT * FROM `armymem` WHERE id=\"" + us.iddb + "\" LIMIT 1;");
                    red.first();
                }
                ...
                for (i = 0; i < len; i++) {
                    ...
                    /* lever */
                      us.lever[i] = (short) Integer.parseInt(jobj.get("lever").toString());
                    /* xp % */
                ...
                System.out.println("Size item:"+jarr.size());
                ...
                if (Until.compare_Day(new Date(), newLongin)) {
                 ...
                 //   us.updateLuong(100);
                   ...
                   // us.updateXu(100000);
                 // us.sendMSSToUser(null, "Chúc mừng bạn nhận được 100 lượng và 100k xu.");
                   
                  
                }
               
                us.sendMSSToUser(null, "Mã quà tặng cho anh em mới chơi là tanthu!");
    //            us.sendMSSToUser(null, "Bạn có: "+us.xu+" xu, "+us.luong+" lượng");
                us.sendMSSToUser(null, "Nếu lỗi trang bị, vào cửa hàng mua tháo trang bị và thoát game vào lại là được!");
            //     us.sendMSSToUser(null, "Server tự động bảo trì lúc 00h thời gian bảo trì 30s. Để tránh mất exp gần tới giờ bảo trì thì thoát game lưu dữ liệu!");
            
              //  us.updateXukhoa(-1);
               
                red.close();
                // Set online
                SQLManager.stat.executeUpdate("UPDATE `armymem` SET `online`=1,`newOnline`='" + Until.toDateString(new Date()) + "', `idapp`=" + cl.id + " WHERE `id`=" + us.iddb + " LIMIT 1;");
                // Luyen tap manager
    ...
    protected int flushCache() {
        ...
        try {
            ...
            for (int i = 0; i < this.nvStt.length; i++) {
             ...
             //sk
            SQLManager.stat.executeUpdate("UPDATE `armymem` SET `sk`=" + this.eventScore + ", `baodanhsk` = '" + Until.toDateString(this.baodanhsk) + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            SQLManager.stat.executeUpdate("UPDATE `armymem` SET `nvCSinh`='" + Jarr1.toJSONString() + "', `CSinh` = '" + this.csinh + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            // Xu, luong, ...
            SQLManager.stat.executeUpdate("UPDATE `armymem` SET `xu`='" + this.xu + "',`luong`='" + this.luong +"',`dvong`='" + this.dvong + "',`NVused`='" + (this.nv + 1) + "',`sttnhanvat`='" + nvstt + "',`x2XPTime`='" + Until.toDateString(xpX2Time) + "',`sk` = " + this.eventScore + " ,`nvXPMax`='" + nvXPMax + "',`xpMax`='" + xpMax + "' WHERE `id`=" + this.iddb + " LIMIT 1;");
            ...
            if (hopNgocTB == null && hopNgocItemArray.size() == 1) {
                ...
                if (rdE.entry.id >= 50) {
                    ...
                    if (rdE.entry.id == 110) {
                        ...
                        ds = ms.writer();
                        ds.writeByte(0);
                        ds.writeUTF("Ban muon mo ruong nay khong?");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                    if (rdE.entry.id == 110) {
                        ...
                        for (int i = 0; i <rdE.numb; i++) {
                            byte pre = (byte) Until.nextInt(100);
                            //xu 30%
                            if (pre < 30) {
                                int xuup = new int[]{10000, 50000, 100000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xuup)+" Xu";
                                this.updateXu(xuup);
                                //30% luong
                            } else if (pre < 60) {
                                int luongup = new int[]{100, 200, 300}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(luongup)+" Lượng";
                                this.updateLuong(luongup);
                                //20% xp
                            } else if (pre < 80) {
                                int xpup = new int[]{10000, 50000, 100000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xpup)+" XP";
                                this.updateXP(xpup, false);
                                //20% ngoc
                            } else if (pre < 100) {
                                byte idI = (byte) Until.nextInt(99, 104);
                                byte xI = (byte) Until.nextInt(1, 5);
                                nameI = xI+""+SpecialItemData.getSpecialItemById(idI).name;
                                this.updateSpecialItem(idI, 1);
                            }
                            ms = new Message(45);
                            ds = ms.writer();
                            ds.writeUTF("Bạn nhận được "+ nameI);
                            ds.flush();
                            this.sendMessage(ms);
                        }
                        return;
                    ...
                    if (rdE.entry.id == 74) {
                        ...
                        for (int i = 0; i <rdE.numb; i++) {
                            ...
                            //xu 15%
                            if (pre < 10) {
                                int xuup = new int[]{100000, 500000, 1000000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xuup)+" Xu";
                                this.updateXu(xuup);
                                //30% luong
                            } else if (pre < 30) {
                                int luongup = new int[]{7, 30, 110}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(luongup)+" Lượng";
                                this.updateLuong(luongup);
                                //10% xp
                            } else if (pre < 10) {
                                int xpup = new int[]{2000, 3000, 5000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xpup)+" XP";
                                this.updateXP(xpup, false);
                                //20% ngoc
                            } else if (pre < 100) {
                        ...
                        for (int i = 0; i <rdE.numb; i++) {
                            ...
                            //xu 30%
                            if (pre < 30) {
                                int xuup = new int[]{1000, 2000, 5000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xuup)+" Điểm 8/3";
                                this.updateEventScore(xuup);
                                //30% luong
                            } else if (pre < 60) {
                                int luongup = new int[]{100, 200, 300}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(luongup)+" Lượng";
                                ...
                                int xpup = new int[]{20000, 50000, 100000}[Until.nextInt(3)];
                                nameI = Until.getStrungNum(xpup)+" XP";
                                this.updateXP(xpup, false);
                                //20% ngoc
                            } else if (pre < 100) {
                                byte idI = (byte) Until.nextInt(88, 93);
                                byte xI = (byte) Until.nextInt(1, 5);
                                nameI = xI+""+SpecialItemData.getSpecialItemById(idI).name;
                                this.updateSpecialItem(idI, 1);
                            }
                    ...
                    if (rdE.entry.id == 85) {
                        if (rdE.numb == 1) {
                            if (this.getXu() >= 500000) {
                                ...
                                this.updateRuong(null, null, -1, null, hopNgocItemArray);
                                this.updateSpecialItem(87, 1);
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Bạn nhận được bánh dày hoàn thiện");
                                ds.flush();
                                this.sendMessage(ms);
                                return;
                            } else {
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Yêu cầu đủ 500k xu");
                                ds.flush();
                                this.sendMessage(ms);
                                return;
                            }
                         ...
                         if (rdE.entry.id == 84) {                           
                            if (rdE.numb == 1) {
                            if (this.getLuong() >= 500) {
                                this.updateLuong(-500);
                                this.updateRuong(null, null, -1, null, hopNgocItemArray);
                                this.updateSpecialItem(86, 1);
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Bạn nhận được bánh trưng hoàn thiện");
                                ds.flush();
                                this.sendMessage(ms);
                                return;
                            } else {
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Yêu cầu đủ 500 lượng");
                                ds.flush();
                                this.sendMessage(ms);
                                return;
                               }
                    ...
                    if (rdE.entry.id == 86 || rdE.entry.id == 87) {
                        this.hopNgocAction = 6;
                        ms = new Message(17);
                        ds = ms.writer();
                        ds.writeByte(0);
                    ...
                    if (rdE.entry.id == 89) {
                        ...
                        ds = ms.writer();
                        ds.writeByte(0);
                        ds.writeUTF("Bạn chắc chắn muốn vượt cấp?");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                      if (rdE.entry.id == 118) {
                        ...
                        ds = ms.writer();
                        ds.writeByte(0);
                        ds.writeUTF("Bạn có muốn Max Item không???");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                    ...
                    if (rdE.entry.id > 98 && rdE.entry.id < 105) {
                        this.hopNgocAction = 6;
                        ms = new Message(17);
                        ds = ms.writer();
                        ds.writeByte(0);
                        ds.writeUTF("Sử dụng viên mới sẽ mất tác dụng của viên cũ. Bạn có xác nhận dùng viên mới không!");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                    if (rdE.numb == 5 && rdE.entry.id >= 105 && rdE.entry.id <= 109) {
                        int idg  = rdE.entry.id+5;
                        updateRuong(null, null, -1, null, hopNgocItemArray);
                        boolean sce = Until.nextInt(100)<10;
                        if (sce){
                            this.updateSpecialItem(idg, 1);
                        }
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn ghép "+(sce?("thành công"):("thất bại rồi"))+"!");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                    ...
                    ds = ms.writer();
                    ds.writeByte(0);
                    ds.writeUTF(String.format(GameString.hopNgocNC(),
                    (100 - (rdE.entry.id % 10) * 10) + "%"));
                    ds.flush();
                    sendMessage(ms);
                    return;
                } else if(rdE.numb == 5 && ((rdE.entry.id + 1) % 10 == 0)) {
                    this.hopNgocAction = 7;
                    ms = new Message(17);
                    ds = ms.writer();
                    ds.writeByte(0);
                    ds.writeUTF("Tỉ lệ ghép ngọc 11 là 20% bạn có muốn ghép không?");
                    ds.flush();
                    sendMessage(ms);
                    return;
                }
            
             } else if (hopNgocTB == null && hopNgocItemArray.size() > 1) {
                int[] arItem = new int[SpecialItemData.entrys.size()];
                for (byte i = 0; i < hopNgocItemArray.size(); i++) {
                    arItem[hopNgocItemArray.get(i).entry.id] = hopNgocItemArray.get(i).numb;
                }
               if (hopNgocItemArray.size() == 4) {
                    if (arItem[79] == 5 && arItem[80] == 5 && arItem[81] == 5 && arItem[83] == 5) {
                        this.updateRuong(null, null, -1, null, hopNgocItemArray);
                        this.updateSpecialItem(85, 1);
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn nhận được bánh dày");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
             
                    if (arItem[79] == 10 && arItem[80] == 10 && arItem[81] == 10 && arItem[83] == 10) {
                        this.updateRuong(null, null, -1, null, hopNgocItemArray);
                        this.updateSpecialItem(84, 1);
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn nhận được bánh trưng");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
             
                   /* if (arItem[75] == 10 && arItem[76] == 10) {
                        this.updateRuong(null, null, -1, null, hopNgocItemArray);
                        this.updateSpecialItem(90, 1);
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn nhận được 1 Đoá Hoa 8/3");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }     */               
                } else if(hopNgocItemArray.size() == 2) {
                    if (arItem[85] >= 1 && arItem[82] >= 1 && arItem[82] == arItem[85]) {
                        this.updateRuong(null, null, -1, null, hopNgocItemArray);
                        this.updateSpecialItem(87, arItem[82]);
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn nhận được "+ arItem[82] +" bánh dày hoàn thiện");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                    if (arItem[84] >= 1 && arItem[82] >= 1 && arItem[82] == arItem[84]) {
                        this.updateRuong(null, null, -1, null, hopNgocItemArray);
                        this.updateSpecialItem(86, arItem[82]);
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Bạn nhận được "+ arItem[82] +" bánh trưng hoàn thiện");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
            ...
            switch (this.hopNgocAction) {
                ...
                case 6: {
                    ...
                    if (rdE.entry.id == 50) {
                        ...
                        this.pointAdd[this.nv][2] = 0;
                        this.pointAdd[this.nv][3] = 0;
                        this.pointAdd[this.nv][4] = 0;
                        sendPointAddInfo();
                        updateRuong(null, null, -1, null, hopNgocItemArray);
                        ms = new Message(45);
                        ds = ms.writer();
                        ...
                        if (rdE.numb == 10) {
                            byte idItem = new byte[]{99,100,101,102,103,104}[Until.nextInt(6)];
                            this.updateSpecialItem(idItem, 5);
                            updateRuong(null, null, -1, null, hopNgocItemArray);
                            ms = new Message(45);
                            ds = ms.writer();
                            ...
                            ms = new Message(45);
                            ds = ms.writer();
                            ds.writeUTF(String.format(GameString.missionComplete(), Until.getStrungNum(xp) + "Điểm"));
                            ds.flush();
                            sendMessage(ms);
                        }
                    }
                    if (rdE.entry.id == 86) {
                        if (rdE.numb == 100 || rdE.numb == 150 || rdE.numb == 200 || rdE.numb == 255) {
                            ...
                            for (byte i = 0; i < 5; i++) {
                                ...
                                switch (rdE.numb) {
                                    case 100:
                                        rdE2.vipLevel = 1;
                                        for (byte j = 0; j < 5; j++) {
                                            rdE2.invAdd[j] = (short) Until.nextInt(20, 25);
                                            rdE2.percenAdd[j] = (short) Until.nextInt(10, 14);
                                        }
                                        break;
                                    case 150:
                                        rdE2.vipLevel = 2;
                                        for (byte j = 0; j < 5; j++) {
                                            rdE2.invAdd[j] = (short) Until.nextInt(25, 30);
                                            rdE2.percenAdd[j] = (short) Until.nextInt(12, 16);
                                        }
                                        break;
                                    case 200:
                                        rdE2.vipLevel = 3;
                                        for (byte j = 0; j < 5; j++) {
                                            rdE2.invAdd[j] = (short) Until.nextInt(30, 35);
                                            rdE2.percenAdd[j] = (short) Until.nextInt(14, 18);
                                        }
                                        break;
                                    case 255:
                                        rdE2.vipLevel = 4;
                                        for (byte j = 0; j < 5; j++) {
                                            rdE2.invAdd[j] = (short) Until.nextInt(35, 40);
                                            rdE2.percenAdd[j] = (short) Until.nextInt(16, 20);
                                        }
                                        break;
                                }
                            ...
                            sendMessage(ms);
                                ...
                                int xp = rdE.numb * 1000;
                                updateEventScore(xp); //false);
                                this.updateRuong(null, null, -1, null, hopNgocItemArray);
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF(String.format(GameString.missionComplete(), xp + "Điểm"));
                                ds.flush();
                            sendMessage(ms);
                        }
                    }
                     if (rdE.entry.id == 118) {
                        ...
                        ms = new Message(45);
                        ds = ms.writer();
                        ds.writeUTF("Add Max Item thành công, thoát ra vào lại để cập nhật");
                        ds.flush();
                        sendMessage(ms);                     
                    }
                    
                    else if (rdE.entry.id == 89) {
                        if (this.nvCSinh[this.nv] == 0 && this.lever[this.nv] <= 1999) {
                            ms = new Message(45);
                            ds = ms.writer();
                            ds.writeUTF("Level của bạn cần đạt 2000 để vượt cấp!");
                            ds.flush();
                            sendMessage(ms);
                        } else if (this.nvCSinh[this.nv] >= 999) {
                            ms = new Message(45);
                            ds = ms.writer();
                            ds.writeUTF("Số lần dùng tính năng này là 999 lần!");
                            ds.flush();
                            sendMessage(ms);
                        } else if (this.nvCSinh[this.nv] == 0) {
                            updateRuong(null, null, -1, null, hopNgocItemArray);
                            int xpnew = this.xp[this.nv] - (2000 * 1999 * 500);
                            this.lever[this.nv] = 2000;
                            this.xp[this.nv] = 0;
//                            if (this.nvCSinh[this.nv] == 0) {
//                                this.point[this.nv] += 2;
//                            } else {
...
//                            }
                            ...
                            ms = new Message(45);
                            ds = ms.writer();
                            ds.writeUTF("Chúc mừng bạn vượt cấp thành công");
                            ds.flush();
                            sendMessage(ms);
                        } else {
                            if (this.lever[this.nv] < 2000) {
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Để chuyển sinh bạn cần phải đạt cấp 2.000");
                                ds.flush();
                                sendMessage(ms);
                            } else {
                                updateRuong(null, null, -1, null, hopNgocItemArray);
                                int xpnew = this.xp[this.nv] - (2000 * 2000 * 500);
    ...
    //                            }
                                ...
                                ms = new Message(45);
                                ds = ms.writer();
                                ds.writeUTF("Chúc mừng bạn chuyển sinh thành công");
                                ds.flush();
                                sendMessage(ms);
                            }
                        }
                    }
                    else if (rdE.entry.id > 98 && rdE.entry.id < 105) {
                ...
                case 7:
                    ...
                    boolean sce = Until.nextInt(100)<10;
                    if (sce){
                        byte id = (byte)((hopNgocItemArray.get(0).entry.id+1)/10);
                        this.updateSpecialItem(104+id, 1);
                    }
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Bạn ghép "+(sce?("thành công"):("thất bại rồi"))+"!");
    ...
    protected void quaysoMessage(Message ms) throws IOException {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (type == 0) {
                ...
                numb = (new int[]{100, 50, 70, 150})[Until.nextInt(new int[]{400, 300, 200, 100})];
                if (i == lucKyNum) {
                    this.updateItem(idItem, numb);
                }
            }
            if (type == 1) {
                numb = (new int[]{50000, 100000, 500000, 1000000})[Until.nextInt(new int[]{400, 300, 200, 100})];
                if (i == lucKyNum) {
                    this.updateXu(numb);
                }
            }
            if (type == 2) {
                numb = (new int[]{100000, 200000, 300000, 500000})[Until.nextInt(new int[]{400, 300, 200, 100})];
                if (i == lucKyNum) {
    ...
    protected void specialShopMessage(Message ms) throws IOException {
        ...
        if (type == 1) {
            ...
            if (spE.id == 51) {
                ...
                ms = new Message(45);
                ds = ms.writer();
                ds.writeUTF("Đã tháo hết trang bị đang mặc.Vui lòng thoát game đăng nhập lại để hoàn tất!");
                ds.flush();
                sendMessage(ms);
                return;
            }
            ...
            if (spE.id == 55) {
               ...
               if(buyLuong==1){
                ...
                if (this.luong >= gia) {
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Đổi thành công "+gia*100000+" xu không khoá!");
                    ds.flush();
                    sendMessage(ms);
                    updateLuong(-gia);
                    return;
        ...
        try {
            ...
            if (jSize > ServerManager.max_friends) {
                ...
                if (jSize > 0) {
                    ...
                    jarr = (JSONArray) JSONValue.parse("[" + ids + "]");
                }
                if (found) {
                    ds.writeByte(1);
                } else {
    ...
    protected void viewTTMessage(Message ms) throws IOException {
        ...
        try {
            ...
            DataOutputStream ds = ms.writer();
            ds.writeInt(ids);
            ResultSet red = SQLManager.stat.executeQuery("SELECT `user` FROM `user` WHERE user_id=\"" + ids + "\" LIMIT 1;");
            red.first();
            ds.writeUTF(red.getString("user"));
            red.close();
            red = SQLManager.stat.executeQuery("SELECT `xu`,`luong`,`NVused`,`dvong`,`top` FROM `armymem` WHERE id=\"" + ids + "\" LIMIT 1;");
            red.first();
            ds.writeInt(red.getInt("xu"));
            ...
            red.close();
            red = SQLManager.stat.executeQuery("SELECT `NV" + nvS + "` FROM `armymem` WHERE id=\"" + ids + "\" LIMIT 1;");
            red.first();
    ...
    public synchronized void updateSpecialItem(int id, int numb) throws IOException {
    ...
    public void updateQua_Start(int addnQua, int time, boolean start) throws IOException {
        ...
        if (start && !this.startQua) {
            ...
            ds.writeByte(-1);
            ds.writeByte(time);
            ds.writeUTF("Bạn có " + this.moQua + " lượt chọn quà miễn phí. Cần 5K CUP để mở thêm các hộp quà khác!");
            ds.flush();
            this.sendMessage(ms);
                ...
                public void run() {
                    try {
                        while (true) {
                            ...
                            if (timeQua <= 0 || !startQua || n_open > 12) {
                                byte id = 0;
                                String name;
                                Message ms = new Message(-17);
                                DataOutputStream ds = ms.writer();
                                ...
                                for (byte index = 0; index < 12; index++) {
                                    ...
                                    switch (Until.nextInt(5)) {
                                        ...
                                        case 1:
                                            id = 56;
                                            type = 2;
                                            int[] nextXP = new int[]{10000, 20000, 30000, 40000, 50000};
                                            int xpUp = nextXP[Until.nextInt(nextXP.length)];
                                            name = Until.getStrungNum(xpUp) + " xp";
                                            break;
    ...
    protected void giftAfterFight(Message ms) throws IOException {
        ...
        if (index == -2) {
            ...
            if (this.moQua > 0) {
                this.moQua--;
            } else if (this.getDvong() >= 5000) {
                this.updateDvong(-5000);
            } else {
                this.startQua = false;
                return;
            }
    ...
    public boolean getisIdEquip2(int type) {
        for (int i = 0; i < Equip2.size(); i++) {
