...
public class Room {
    ...
    public Room(int id, int type, int maxEntrys, int ntype) {
        ...
        boolean isLH = false, isSiuBoss = false;

        
        if (type == 5 && ntype == 9) {
            this.slMap.add((byte)30);
            isSiuBoss = true;
        } else {
        ...
        for (int i = 0; i < maxEntrys; i++) {
            this.entrys[i] = new FightWait(this, this.type, (byte) i, ServerManager.maxPlayers, maxPlayerInit, map, (byte) Until.nextInt(0, 2), isLH, isSiuBoss);
        }
    }
    ...
    protected int getFully() {
        ...
        synchronized (entrys) {
            ...
            int perCent = (player * 1000) / maxPlayers;
