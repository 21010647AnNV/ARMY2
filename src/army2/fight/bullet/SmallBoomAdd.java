...
public class SmallBoomAdd extends Bullet {
    ...
    @Override
    public void nextXY() throws IOException {
        super.nextXY();
        if (super.collect && X > 0 && X < fm.mapMNG.Width && Y < fm.mapMNG.Height) {
            Player players;
            try {
                players = new SmallBoom(fm, (byte) 11, "Small Boom", (byte) fm.allCount, 500 + (fm.getLevelTeam() * 10), X, Y);
                bullMNG.addbos.add(new BulletManager.AddBOSS(players, 2));
