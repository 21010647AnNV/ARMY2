...
public class FightWait {
    ...
    public boolean isLH;
    public byte ntLH;
    public byte[] LHMap = new byte[]{30, 31, 32, 33, 34, 35, 36, 37, 38};
    public boolean isSiuBoss;
   
    public FightWait(Room parent, byte type, byte id, byte maxPlayers, byte maxPlayerInit, byte map, byte teaFree, boolean isLH, boolean isSiuBoss) {
        this.parent = parent;
        this.id = id;
    ...
    public void enterFireOval(User us) throws IOException {
        ...
        synchronized (this.players) {
            ...
            for (byte i = 0; i < this.players.length; i++) {
                if (this.players[i] != null) {
                    ...
                    ds.writeInt(0);
                    // level
                    ds.writeByte(us0.getLevel() > 255 ? 255 : us0.getLevel());
                    // Nhan vat
        ...
        if (this.isSiuBoss) {
            ms = new Message(45);
            ds = ms.writer();
            ds.writeUTF("Chào bạn đến với chế độ siêu boss ^_^");
            ds.flush();
            us.sendMessage(ms);
       }
        if (this.map == 34) {
            ms = new Message(45);
            ds = ms.writer();
            ds.writeUTF("Chào bạn đến với chế độ siêu T-Rex ^_^");
            ds.flush();
            us.sendMessage(ms);    
        }    
    ...
    public void setMoneyMessage(User us, Message ms) throws IOException {
        ...
        if(us.getXu()<xu) {
            ...
            ds.writeUTF("Bạn chỉ có "+us.getXu()+" xu không khoá");
            ds.flush();
            us.sendMessage(ms);
            return;
        }
    ...
    public void startMessage(ClientEntry cl) throws IOException {
      ...
      /*  if (isSiuBoss) {
            ...
            if(hours < 17 || hours >= 23){
                ms = new Message(45);
                ds = ms.writer();
                ds.writeUTF("Siêu boss chỉ mở từ 17h-23h!");
                ds.flush();
                cl.sendMessage(ms);
                return;
            }
...
//        }
        ...
        for (byte i = 0; i < this.players.length; i++) {
            ...
            if (type == 6) {
                byte j = (byte)((i%2==0)?0:1);
                for (; j < this.players.length; j+=2) {
                    User pl2 = this.players[j];
                    if (pl2 == null || pl2.getClan() != pl.getClan())
                        continue;
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Đội phải ở cùng phe!");
                    ds.flush();
                    cl.sendMessage(ms);
                    return;
                }
            ...
            for (j = 0; j < 8; j++) {
                byte itemSelect = (byte) this.item[i][j];
